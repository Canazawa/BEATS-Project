...
HRESULT CD3DXMESH_ANIM::InitShader()
{
	...
	//ブロブからバーテックスシェーダー作成
	if (FAILED(D3DX11CompileFromFile(L"hlsl/MESH_ANIME.hlsl", NULL, NULL, "VS", "vs_5_0", 0, 0, NULL, &pCompiledShader, &pErrors, NULL)))
	{
	...
	SAFE_RELEASE(pErrors);
	SAFE_RELEASE(pCompiledShader);
	if (FAILED(D3DX11CompileFromFile(L"hlsl/MESH_ANIME.hlsl", NULL, NULL, "VS_NoTex", "vs_5_0", 0, 0, NULL, &pCompiledShader, &pErrors, NULL)))
	{
		MessageBox(0, L"hlsl読み込み失敗", NULL, MB_OK);
		return E_FAIL;
	}
	...
	//ブロブからピクセルシェーダー作成
	if (FAILED(D3DX11CompileFromFile(L"hlsl/MESH_ANIME.hlsl", NULL, NULL, "PS", "ps_5_0", 0, 0, NULL, &pCompiledShader, &pErrors, NULL)))
	{
		MessageBox(0, L"hlsl読み込み失敗", NULL, MB_OK);
		return E_FAIL;
	}
	...
	//テクスチャーなし
	if (FAILED(D3DX11CompileFromFile(L"hlsl/MESH_ANIME.hlsl", NULL, NULL, "PS_NoTex", "ps_5_0", 0, 0, NULL, &pCompiledShader, &pErrors, NULL)))
	{
		MessageBox(0, L"hlsl読み込み失敗", NULL, MB_OK);
		return E_FAIL;
	}
...
D3DXMATRIX CD3DXMESH_ANIM::m_View;
D3DXMATRIX CD3DXMESH_ANIM::m_Proj;
D3DXVECTOR3 CD3DXMESH_ANIM::m_Light;
D3DXVECTOR3 CD3DXMESH_ANIM::m_Eye;

void CD3DXMESH_ANIM::SetCamera(D3DXMATRIX& view, D3DXMATRIX& proj, D3DXVECTOR3& vLight, D3DXVECTOR3& vEye)
{
	m_View = view;
	m_Proj = proj;
	m_Light = vLight;
...
void CD3DXMESH_ANIM::Render(D3DXVECTOR3& pos, D3DXVECTOR3& rot, D3DXVECTOR3& scale)
{
	//プリミティブ・トポロジーをセット
	m_pDeviceContext11->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);

	D3DXMATRIX World, Tran, Yaw, Pitch, Roll,Scale;
	...
	m_fYaw = rot.y;
	m_fRoll = rot.z;
	m_fScale = D3DXVECTOR3(scale.x, scale.y, scale.z);

	// ワールドトランスフォーム
	D3DXMatrixScaling(&Scale, m_fScale.x, m_fScale.y, m_fScale.z);
	D3DXMatrixTranslation(&Tran, m_vPos.x, m_vPos.y, m_vPos.z);
	D3DXMatrixRotationX(&Pitch, m_fPitch);
	D3DXMatrixRotationY(&Yaw, m_fYaw);
	D3DXMatrixRotationZ(&Roll, m_fRoll);
	D3DXVec3TransformCoord(&m_AxisX, &D3DXVECTOR3(1, 0, 0), &Pitch);
	D3DXVec3TransformCoord(&m_AxisY, &D3DXVECTOR3(0, 1, 0), &Yaw);
	D3DXVec3TransformCoord(&m_AxisZ, &D3DXVECTOR3(0, 0, 1), &Roll);
	World = Scale * Yaw * Pitch * Roll * Tran;
...
void CD3DXMESH_ANIM::MoveJump(float Length)
{
	m_vPos += m_AxisY * Length;
}