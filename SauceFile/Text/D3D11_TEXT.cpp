...
HRESULT D3D11_TEXT::Init(ID3D11DeviceContext* pContext, DWORD width, DWORD height, float size, D3DXVECTOR4 vColor)
{
	...
	for (int i = 0; i < 95; i++)
	{
		m_fKerning[i] = WDIMENSION; // 一文字の間隔
	}
	//デバイスとコンテキストをコピー
	...
	//フォントのテクスチャーを作成
	if(FAILED(D3DX11CreateShaderResourceViewFromFile(m_pDevice, L"Font/ascii.png", NULL, NULL, &m_pAsciiTexture, NULL )))
	{
		return E_FAIL;
	}
	...
	//ブロブからバーテックスシェーダー作成
	if (FAILED(D3DX11CompileFromFile(L"hlsl/D3D11_TEXT.hlsl", NULL, NULL, "VS", "vs_5_0", 0, 0, NULL, &pCompiledShader, &pErrors, NULL)))
	{
	...
	if( FAILED( m_pDevice->CreateInputLayout( layout, numElements, pCompiledShader->GetBufferPointer(), pCompiledShader->GetBufferSize(), &m_pVertexLayout ) ) )
		return FALSE;
	//ブロブからピクセルシェーダー作成
	if(FAILED(D3DX11CompileFromFile(L"hlsl/D3D11_TEXT.hlsl",NULL,NULL,"PS","ps_5_0",0,0,NULL,&pCompiledShader,&pErrors,NULL)))
	{
		MessageBox(0,L"hlsl読み込み失敗",NULL,MB_OK);
		return E_FAIL;
    }
...
//
//
void D3D11_TEXT::Render(char* text, int x, int y, float alpha)
{
	// ビュートランスフォーム（視点座標変換）
	...
	for (int i = 0; i < strlen(text); i++)
	{
		char font = text[i];
		int index = font - 32;
		RenderFont(index,x,y, alpha);
		x += m_fKerning[index]; // 文字列分X軸にずらす
	}
}
//
//
//
void D3D11_TEXT::RenderFont(int FontIndex,int x,int y,float alpha)
{
	//ワールド変換
	D3DXMATRIX mWorld;
